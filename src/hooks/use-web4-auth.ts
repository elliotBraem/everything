...
export class BrowserWeb4Auth implements AuthMethod {
    constructor(
        public driver: BrowserWeb4Auth.Driver,
    ) { }
    ...
    async start(): Promise<AuthResult> {
        ...
        if (accountId && privateKey) {
            ...
            if (storedCredentials) {
                const { jazzAccountID, jazzAccountSecret } = JSON.parse(storedCredentials);
                return {
                    type: "existing",
                    credentials: {
                ...
                return {
                    ...
                    saveCredentials: async (credentials: {
                        ...
                        localStorage.setItem(`jazz-web4-${accountId}`, JSON.stringify({
